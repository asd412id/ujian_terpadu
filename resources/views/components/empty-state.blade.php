...
@php
    $icons = [
        'inbox' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-2.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"/>',
...
<div class="{{ $compact ? 'py-8' : 'py-14' }} flex flex-col items-center justify-center text-center px-4">
    <div class="{{ $compact ? 'w-12 h-12 mb-3' : 'w-16 h-16 mb-4' }} rounded-full bg-gray-100 flex items-center justify-center">
        <svg class="{{ $compact ? 'w-6 h-6' : 'w-8 h-8' }} text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            {!! $svgPath !!}
        </svg>
    </div>
    <p class="{{ $compact ? 'text-sm' : 'text-base' }} font-medium text-gray-500">{{ $title }}</p>
    @if($subtitle)
        <p class="{{ $compact ? 'text-xs' : 'text-sm' }} text-gray-400 mt-1 max-w-sm">{{ $subtitle }}</p>
    @endif
    @if($slot->isNotEmpty())
        <div class="mt-3">
            {{ $slot }}
