...
<div class="space-y-5" x-data="trashManager(@js($trashedNarasi->pluck('id')->values()), @js($allFilteredIds ?? []), 'narasi')">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
            ...
            <form action="{{ route('pembuat-soal.narasi.empty-trash') }}" method="POST"
            ...
            @endif
            <a href="{{ route('pembuat-soal.soal.index', ['tab' => 'narasi']) }}" class="btn-secondary inline-flex items-center gap-1.5">Kembali</a>
        </div>
    </div>
    ...
    <div x-show="selectionCount > 0" x-cloak class="sticky top-20 z-30 card bg-blue-50 border border-blue-200 shadow-sm flex flex-col gap-3 p-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div class="flex items-center gap-2 flex-wrap">
                <form method="POST" action="{{ route('pembuat-soal.narasi.bulk-restore') }}"
                ...
                <form method="POST" action="{{ route('pembuat-soal.narasi.bulk-force-delete') }}"
                ...
                <tr class="hover:bg-gray-50 transition-colors" :class="rowClass('{{ $narasi->id }}')">
                    ...
                    <td class="px-4 py-3 text-right whitespace-nowrap">
                        <div class="flex items-center justify-end gap-1">
                            <form action="{{ route('pembuat-soal.narasi.restore', $narasi->id) }}" method="POST"
    ...
    <div class="card text-center py-16">
        ...
        <a href="{{ route('pembuat-soal.soal.index', ['tab' => 'narasi']) }}" class="text-blue-600 hover:underline text-sm">Kembali ke Bank Soal</a>
    </div>
    @endif
</div>
