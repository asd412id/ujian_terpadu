...
<div class="space-y-5" x-data="trashManager(@js($trashedSoal->pluck('id')->values()), @js($allFilteredIds ?? []), 'soal')">
    ...
    <div x-show="selectionCount > 0" x-cloak class="sticky top-20 z-30 card bg-blue-50 border border-blue-200 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-3">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            <form method="POST" action="{{ route('pembuat-soal.soal.bulk-restore') }}" @submit.prevent="if(await $store.confirmModal.open({title:'Pulihkan Soal',message:'Pulihkan ' + selectionCount + ' soal terpilih?',confirmText:'Ya, Pulihkan'})) submitBulk($el)">
                @csrf
            ...
            <form method="POST" action="{{ route('pembuat-soal.soal.bulk-force-delete') }}" @submit.prevent="if(await $store.confirmModal.open({title:'Hapus Permanen',message:'HAPUS PERMANEN ' + selectionCount + ' soal terpilih?\n\nTindakan ini TIDAK DAPAT dibatalkan!',confirmText:'Ya, Hapus Permanen',danger:true})) submitBulk($el)">
                @csrf @method('DELETE')
    ...
    <div class="card overflow-x-auto">
        ...
        <table class="w-full text-sm text-left">
            <thead><tr class="border-b border-gray-200 text-xs text-gray-500 uppercase tracking-wider"><th class="px-4 py-3 w-10"></th><th class="px-4 py-3">Pertanyaan</th><th class="px-4 py-3">Kategori</th><th class="px-4 py-3">Jenis</th><th class="px-4 py-3">Dihapus</th><th class="px-4 py-3 text-right">Aksi</th></tr></thead>
            <tbody class="divide-y divide-gray-100">
                @foreach($trashedSoal as $soal)
                <tr class="hover:bg-gray-50 transition-colors" :class="isSelected('{{ $soal->id }}') ? 'bg-blue-50' : ''">
                    <td class="px-4 py-3"><input type="checkbox" value="{{ $soal->id }}" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500" @change="toggle('{{ $soal->id }}')" :checked="isSelected('{{ $soal->id }}')"></td>
                    ...
                    <td class="px-4 py-3 whitespace-nowrap">@php $tipeLabels = ['pg' => ['PG', 'bg-blue-100 text-blue-700'],'pg_kompleks' => ['PG Kompleks', 'bg-purple-100 text-purple-700'],'benar_salah' => ['B/S', 'bg-yellow-100 text-yellow-700'],'menjodohkan' => ['Jodoh', 'bg-green-100 text-green-700'],'isian' => ['Isian', 'bg-orange-100 text-orange-700'],'essay' => ['Essay', 'bg-gray-100 text-gray-700']]; [$tipeLabel, $tipeClass] = $tipeLabels[$soal->tipe_soal] ?? [$soal->tipe_soal, 'bg-gray-100 text-gray-600']; @endphp <span class="text-xs font-semibold px-2 py-0.5 rounded-full {{ $tipeClass }}">{{ $tipeLabel }}</span></td>
                    <td class="px-4 py-3 text-gray-500 whitespace-nowrap text-xs">{{ $soal->deleted_at->diffForHumans() }}</td>
                    <td class="px-4 py-3 text-right whitespace-nowrap"><div class="flex items-center justify-end gap-1"><form action="{{ route('pembuat-soal.soal.restore', $soal->id) }}" method="POST" @submit.prevent="if(await $store.confirmModal.open({title:'Pulihkan Soal',message:'Pulihkan soal ini ke Bank Soal?',confirmText:'Ya, Pulihkan'})) $el.submit()">@csrf<button type="submit" class="text-green-600 hover:text-green-800 hover:bg-green-50 px-2 py-1 rounded text-xs font-medium transition-colors">Pulihkan</button></form><form action="{{ route('pembuat-soal.soal.force-delete', $soal->id) }}" method="POST" @submit.prevent="if(await $store.confirmModal.open({title:'Hapus Permanen',message:'HAPUS PERMANEN soal ini? Tindakan ini tidak dapat dibatalkan!',confirmText:'Ya, Hapus Permanen',danger:true})) $el.submit()">@csrf @method('DELETE')<button type="submit" class="text-red-600 hover:text-red-800 hover:bg-red-50 px-2 py-1 rounded text-xs font-medium transition-colors">Hapus Permanen</button></form></div></td>
                </tr>
                @endforeach
            </tbody>
        </table>
    </div>
    @if($trashedSoal->hasPages())<div>{{ $trashedSoal->links() }}</div>@endif
    @else
    <div class="card text-center py-16"><svg class="w-12 h-12 text-gray-300 mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg><p class="text-gray-500 mb-4">Sampah kosong.</p><a href="{{ route('pembuat-soal.soal.index') }}" class="text-blue-600 hover:underline text-sm">Kembali ke Bank Soal</a></div>
    @endif
</div>
