        ...
        <form method="GET" action="{{ route('dinas.grading') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            ...
            <div class="relative w-full sm:w-auto">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama peserta..."
                       class="w-full sm:w-52 pl-9 pr-8 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                @if(request('search'))
                <a href="{{ route('dinas.grading', request()->except(['search', 'page'])) }}"
