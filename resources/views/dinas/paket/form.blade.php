...
<form action="{{ isset($paket) ? route('dinas.paket.update', $paket->id) : route('dinas.paket.store') }}"
    ...
    <div class="card space-y-4">
        ...
        <div class="space-y-3">
            ...
            <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" name="anti_curang" value="1"
                       {{ old('anti_curang', $paket->anti_curang ?? true) ? 'checked' : '' }}
                       class="w-4 h-4 rounded border-gray-300 text-blue-600">
                <div>
                    <span class="text-sm text-gray-700">Aktifkan mode anti curang</span>
