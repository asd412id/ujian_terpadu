...
<div class="space-y-5" x-data="trashManager(@js($paket->pluck('id')->values()), @js($allFilteredIds ?? []), 'paket')">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
            <form method="GET" action="{{ route('dinas.paket.trash') }}" class="relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari paket..."
                       class="w-full sm:w-48 pl-9 pr-8 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                @if(request('search'))
                <a href="{{ route('dinas.paket.trash') }}" class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
