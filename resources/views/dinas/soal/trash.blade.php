        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="space-y-1">
                ...
                <div class="flex items-center gap-3 text-xs">
                    <span x-show="selectAllFilteredMode" x-cloak class="inline-flex items-center gap-1 rounded-full bg-blue-100 text-blue-700 px-2.5 py-1 font-medium">
                        Mode semua hasil filter aktif
                    </span>
            ...
            <div class="flex items-center gap-2 flex-wrap">
                <form method="POST" action="{{ route('dinas.soal.bulk-restore') }}"
                    ...
                    @csrf
                    <template x-for="id in selectedIds()" :key="'restore-' + id">
                ...
                <form method="POST" action="{{ route('dinas.soal.bulk-force-delete') }}"
                    ...
                    @csrf
                    @method('DELETE')
                    <template x-for="id in selectedIds()" :key="'delete-' + id">
                        <input type="hidden" name="ids[]" :value="id">
                    </template>
    ...
    <div class="card overflow-x-auto">
        ...
        <table class="w-full text-sm text-left">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors" :class="rowClass('{{ $soal->id }}')">
                    <td class="px-4 py-3">
                        <input type="checkbox" value="{{ $soal->id }}"
                    ...
                    <td class="px-4 py-3 max-w-md">
                        ...
                        <button x-show="selectAllFilteredMode" x-cloak type="button"
                                ...
                                @click="toggle('{{ $soal->id }}')"
                                x-text="isExcluded('{{ $soal->id }}') ? 'Masukkan lagi item ini' : 'Kecualikan item ini'"></button>
                    </td>
