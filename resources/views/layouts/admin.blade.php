        ...
        <div class="px-5 py-5 border-b border-white/10">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center flex-shrink-0">
                    <img src="/images/logo.svg" alt="Logo" class="w-7 h-7">
                </div>
                <div>
                    <p class="text-white font-bold text-sm leading-tight">{{ strtoupper(config('app.name')) }}</p>
                    <p class="text-blue-300 text-xs">{{ auth()->user()->sekolah?->nama ?? 'Dinas Pendidikan' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="px-4 py-4 border-t border-white/10">
            <div class="flex items-center gap-3">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('account.edit') }}" class="p-0.5 text-blue-300 hover:text-white transition-colors" title="Pengaturan Akun">
                    ...
                    <form action="{{ route('logout') }}" method="POST" class="flex">
                        @csrf
                        <button type="submit" class="p-0.5 text-blue-300 hover:text-white transition-colors" title="Logout">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
