...
Route::prefix('ujian')->name('ujian.')->group(function () {
    Route::middleware('guest:peserta')->group(function () {
        ...
        Route::post('/login', [PesertaLoginController::class, 'login'])->name('login.post')->middleware('throttle:500,1');
    });
...
// =============================================================
Route::prefix('api/ujian')->name('api.ujian.')->middleware(['throttle:3000,1', 'verify.ujian.token'])->group(function () {
...
Route::prefix('dinas')->name('dinas.')->middleware(['auth', 'role:super_admin,admin_dinas', 'force.password.change'])->group(function () {
    ...
    Route::delete('/laporan/recalculate-progress', [DinasLaporanController::class, 'recalculateClear'])->name('laporan.recalculate-clear');
    ...
    Route::post('/paket/{paket}/sesi/{sesi}/peserta/remove-all', [\App\Http\Controllers\Dinas\SesiUjianController::class, 'removeAllPeserta'])->name('paket.sesi.peserta.remove-all');
    Route::get('/paket/{paket}/sesi/{sesi}/peserta/enrolled-ids', [\App\Http\Controllers\Dinas\SesiUjianController::class, 'enrolledIds'])->name('paket.sesi.peserta.enrolled-ids');
    ...
    // Kartu login peserta (cetak kartu dari halaman peserta)
